...
class SocketManager {
  ...
  Future<void> startServer(String ip, int port) async {
    ...
    debugPrint("Server started on $ip:$port");
  ...
  Future<void> connectToServer(String ip, int port) async {
    ...
    debugPrint("Connected to server at $ip:$port");
    _listenToSocket();
    _startHeartbeat();
  }
  ...
  void sendFileChunk(List<int> chunk) {
    if (_socket != null) {
      final packet = PacketFormatter.encodePacket(PacketType.fileChunk, chunk);
      _socket!.add(packet);
    }
  }
